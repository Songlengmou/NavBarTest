...
import com.anningtex.navbartest.bottom.utils.DotView;
...
//        if(mShiftedColor==0||isViewPager)setItemBackground(config.itemBackGroundRes);
        ...
        if (mShiftedColor != 0 && canChangeBackColor && isSelected) {
            (((BottomNavigationBar) getParent().getParent())).setFirstItemBackgroundColor(mShiftedColor);
        }
        addImageAndText();
    }
    ...
    private void addImageAndText() {
        ...
        mImageView = findViewById(R.id.bar_image);
        mTextView = findViewById(R.id.bar_title);
        ...
        if (iconRes2_selected != 0) {
            mImageView_selected = findViewById(R.id.bar_image_selected);
        ...
        switch (config.switchMode) {
            ...
            case 0:
                ...
                if (!canChangeBackColor) {
                    setItemBackground(config.itemBackGroundRes);
                }
...
//        titleAnimator.alpha(finalAlpha);
    ...
    private void shiftAnim() {
        //no shifting
        if (activeItemWidth == inActiveItemWidth) {
            return;
        }
        updateItemWidth();
    }
    ...
    private void updateItemWidth() {
        ...
        if (isSelected) {
            valueAnimator = ValueAnimator.ofFloat(getWidth(), activeItemWidth);
        } else {
    ...
    private void switchFragment(boolean isSelected) {
        ...
        if (!isSelected) {
            ...
            if (mFragment.isAdded()) {
                ...
                fragmentTransaction.add(getContainerId(), mFragment, tag);
            }
        }
