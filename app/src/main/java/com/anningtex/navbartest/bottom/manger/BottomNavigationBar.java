package com.anningtex.navbartest.bottom.manger;
...
public class BottomNavigationBar extends FrameLayout {
    ...
    public BottomNavigationBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (Build.VERSION.SDK_INT < 21) {
            ...
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, BarUtils.dip2px(context, 1));
...
//        isShy=typedArray.getBoolean(R.styleable.BottomNavigationBar_isShy,false);
    ...
    private void initViewPager(@IdRes int viewpagerId) {
        viewpager = ((Activity) getContext()).findViewById(viewpagerId);
...
//                    }
                    ...
                    if (isSlide) {
                        for (int i = 0; i < itemCounts; i++) {
                            if (!getBottomItem(i).isHasCorrect()) {
                                getBottomItem(i).correctItem(i == position);
                            }
                        }
                    }
                }
                if (positionOffset > 0) {
                    ...
                    if (isSlide) {
                        ...
                        if (canChangeBackColor) {
                            setBackgroundColor(BarUtils.getOffsetColor(positionOffset, getBottomItem(position).getShiftedColor(), getBottomItem(position + 1).getShiftedColor(), 10));
                        }
                    }
                }
            }
...
//                    setItemSelected(position,false);
                    ...
                    if (isSlide) {
                        getChildView().updatePosition(position);
                    } else {
...
//        if(mSwitchMode!=1)return;
        if (currentRadius == 0f) {
            currentRadius = BarUtils.dip2px(getContext(), 10);
        }
        if (maxRadius == 0f) {
            maxRadius = (float) Math.sqrt(getMeasuredHeight() * getMeasuredHeight() + getMeasuredWidth() * getMeasuredWidth());
        }
        if (Build.VERSION.SDK_INT < 21) {
